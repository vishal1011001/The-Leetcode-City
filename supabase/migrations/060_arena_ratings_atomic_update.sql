-- ============================================================
-- 059: Atomic arena_ratings update (v2)
-- ============================================================


-- ── 1. Composite index on arena_active_buffs ─────────────────
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_arena_active_buffs_user_expires
  ON public.arena_active_buffs (user_id, expires_at DESC);


-- ── 2. update_arena_ratings_atomic ───────────────────────────
...
BEGIN
  ...
  -- Guarantee a row exists before the FOR UPDATE so two concurrent
  -- first-ever submissions for the same user cannot both skip the lock.
  INSERT INTO public.arena_ratings (
    user_id, rating, problems_solved, problems_attempted,
    current_streak, best_streak, updated_at
  )
  VALUES (p_user_id, 1200, 0, 0, 0, 0, v_now)
  ON CONFLICT (user_id) DO NOTHING;

  -- Lock the row — all concurrent callers queue here.
  SELECT
  ...
  IF p_is_accepted AND p_is_first_solve THEN
    ...
    -- Use caller-supplied timezone so streak date boundaries are evaluated
    -- in the developer's local date, not always UTC.
    -- The route now passes p_timezone from developers.timezone; this is no
    -- longer dead code.
    v_today     := to_char(v_now AT TIME ZONE p_timezone, 'YYYY-MM-DD');
    v_yesterday := to_char((v_now - INTERVAL '1 day') AT TIME ZONE p_timezone, 'YYYY-MM-DD');

    v_last_solved_date := CASE
      WHEN v_last_solved_at IS NOT NULL
        THEN to_char(v_last_solved_at AT TIME ZONE p_timezone, 'YYYY-MM-DD')
      ELSE NULL
    END;
